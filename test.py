...
def milieu(A, B):
	...
	:Return Value: couple de la forme (x, y) où x et y sont les coordonnées du
				   milieu de A et de B
	"""
	x,y=A[0],A[1]   #On donne la valeurs du couple A a deux variables
	x1,y1=B[0],B[1] #On donne la valeurs du couple B a deux variables
	milieu=((x+x1)/2,(y1+y)/2) #Pour avoir le milieu on additionne les hauteurs et on divise par deux.Me chose pour la largeur.On stock le couple dans milieu que l'on return
...
def distance(A, B):
	...
	return sqrt((x1-x)**2 + (y1-y)**2) #Calcul des distances dans un reperes cartésiens
...
def trace_ligne_brisee(points):
	...
	for position in range(len(points)):
		ligne(points[position-1][0], points[position-1][1], points[position][0], points[position][1], "red")"""

	
	for i in range(len(points)-1): #Fonction itérative , avec un range jusqua longeur liste -1 car on s'arrete a l'avant dernier car on a le [i+1]
		ligne(points[i][0],points[i][1],points[i+1][0],points[i+1][1],couleur='red') # Ici c'est simple on ne se trompe pas avec les indices et on peut tracer la ligne de tout les point consécutifs de la liste
...
def etape_casteljau(points):
	...
	* le point P_i^j, si 0 < j <= N, est le milieu du segement reliant P_i^(j-1) à P_(i+1)^(j-1)
	...
	if len(points)==1:     
		milieudebut.append(points[0])
		milieufin.append(points[0])
		milieufin.reverse()
		return milieufin, milieudebut
	else :
		for i in range(len(points)):
			milieudebut.append(points[0])
			milieufin.append(points[-1])
...
def bezier_casteljau(points):
	...
	if distance_max(points) <= 1:
		...
		milieufin,milieudebut= etape_casteljau(points)
		
		bezier_casteljau(milieufin)
		bezier_casteljau(milieudebut)
